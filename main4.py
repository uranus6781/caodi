...
def parse_url_to_info(url):
    try:
        ...
        time_match = re.search(r"-(\d{4}-\d{2}-\d_2}-\d{4})$", slug)
        if time_match:
...
def push_to_github(all_data):
    ...
    try:
        ...
        files = {FILE_PATH_JSON: json_content, FILE_PATH_M3U: m3u_content}
                ...
                for el in match_elements:
                    ...
                    try:
                        ...
                        for img in images:
                            ...
                            if not src: continue
                            src_lower = src.lower()
                            # BỎ QUA các ảnh có từ khóa liên quan đến giải đấu hoặc logo chung
                            bad_keywords = ["league", "logo-giai", "cup", "icon", "badge", "default", "trend", "trending"]
                    ...
                    # Sửa lỗi gọi hàm: đổi get_team_logo_fallback thành get_team_logo
